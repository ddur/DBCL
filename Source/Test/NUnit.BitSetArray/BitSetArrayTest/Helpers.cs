...
namespace DD.Collections.BitSetArrayTest
{
	...
	public class Helpers
	{
		...
		public void CountOnBits()
		{
			...
			Assert.That(BitSetArray.CountOnBits((IEnumerable<byte>)new byte[3] {
				0,
				255,
				0
			}) == 8);
		...
		public void ValidRangeAndItem()
		{
			...
			Assert.That(BitSetArray.ValidMembers(new int[] { 0, int.MaxValue / 2 }) == true);
			Assert.That(BitSetArray.ValidMembers(new int[] { 0, int.MaxValue - 1 }) == true);
			Assert.That(BitSetArray.ValidMembers(new int[] { 0, int.MaxValue }) == false);
		...
		public void TestVersion()
		{
			...
			BitSetArray test = BitSetArray.Size(1);

			// test.version initial value is int.MaxValue
